...
{
    [AddComponentMenu("Crazy Minnow Studio/SALSA/Addons/Autodesk Character Generator/SALSA 1-Click Autodesk Setup")]
    public class CM_AutodeskSetup : MonoBehaviour
    {
		/// <summary>
		...
		/// <summary>
		/// Configures a complete SALSA with RandomEyes enabled Autodesk character
		/// </summary>
		public void Setup()
        {
            ...
            CM_AutodeskSync autodeskSync; // CM_AutodeskSync
            ...
            autodeskSync = activeObj.AddComponent<CM_AutodeskSync>().GetComponent<CM_AutodeskSync>(); // Add/get CM_AutodeskSync
			autodeskSync.Initialize();
            activeObj.AddComponent<CM_AutodeskLOD>(); // Add CM_AutodeskLOD
            #endregion
            ...
            reShapes.skinnedMeshRenderer = autodeskSync.body; // Set the SkinnedMeshRenderer
            ...
            for (int i = 0; i < reShapes.customShapes.Length; i++)
            {
                if (reShapes.customShapes[i].shapeName.ToLower().Contains("brow") ||
                    reShapes.customShapes[i].shapeName.ToLower().Contains("nostril") ||
                    reShapes.customShapes[i].shapeName.ToLower().Contains("sad") ||
                    reShapes.customShapes[i].shapeName.ToLower().Contains("pityful"))
                {
            ...
            #region CM_AutodeskSync settings
            autodeskSync.salsa3D = salsa3D;
            autodeskSync.randomEyes3D = reEyes;
            #endregion
        }
    }
}